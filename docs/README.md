# Naqi Documentation

> **The AI Workspace Cleaner** — Keep your AI workspace pure.

This is the user-facing documentation for Naqi. Everything here is about
**how to use Naqi** — install it, understand what Token Hygiene does,
debug your config files. For features, pricing, and the product pitch,
see [getnaqi.com](https://getnaqi.com).

---

## Install & update

- [Distribution Guide](distribution.md) — download, install, update paths
- [Uninstall Guide](guides/uninstall.md) — full removal on macOS, Windows, and Linux
- [CHANGELOG](../CHANGELOG.md) — release history
- [SECURITY](../SECURITY.md) — responsible disclosure

## Guides

- [Token Hygiene Guide](guides/token-hygiene.md) — waste-pattern detectors, dashboard visuals, proactive nudges, prompting playbook
- [Pricing & Licensing](guides/pricing-and-licensing.md) — Free vs Pro, pricing, license management, refunds
- [Config Profiles](guides/config-profiles.md) — save, apply, export, and share MCP configurations
- [Keyboard Shortcuts](guides/keyboard-shortcuts.md) — every shortcut in the app
- [Language & Localization](guides/localization.md) — supported languages, how to switch
- [Settings File](guides/settings-file.md) — portable TOML settings, syncing, MDM policy

## Reference

- [Config Formats Reference](technical/config-formats.md) — schemas for every AI client Naqi scans
- [Privacy Policy](legal/privacy-policy.md) — data handling, anonymization, no telemetry
- [Terms of Service](legal/terms-of-service.md) — license grant, acceptable use

## Site content mirror

- [How it works](site/how-it-works.md)
- [Download](site/download.md)
- [Privacy](site/privacy.md)

## Longer reads

- [I Audited My MCP Servers](blog/i-audited-my-mcp-servers.md)
- [MCP Cleanup Tools Compared](blog/mcp-cleanup-tools-compared.md)
